...
def main():
    ...
    for row, n_way in enumerate((5, 7)):
        for col, (backbone, title) in enumerate(zip(BACKBONES, BACKBONE_LABELS)):
            ax = axes[row, col]
            subset = data[(data.n_way == n_way) & (data.backbone == backbone)]
            for method in METHODS:
                ...
                if not line.empty:
                    ax.errorbar(
                        ...
                        linewidth=2,
                        color=colors[method],
                        label=method.replace("net", "Net").title(),
                    )
            ax.set_title(title)
            ax.set_xticks([1, 5, 10])
            ax.grid(axis="y", alpha=0.25)
            if col == 0:
                ax.set_ylabel(f"{n_way}-way accuracy (%)")
            if row == 1:
                ax.set_xlabel("Shots per class (K)")
    handles, labels = axes[0, 0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=4, frameon=False)
    fig.suptitle("Few-shot accuracy by task complexity and backbone", y=0.98)
