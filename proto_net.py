...
def logits_for_episode(model, sx, sy, qx, n_way):
    support = model(sx)
    prototypes = torch.stack([support[sy == i].mean(0) for i in range(n_way)])
    start = time.perf_counter()
    query = model(qx)
    logits = -torch.cdist(query, prototypes).pow(2)
...
def train_episode(model, optimizer, sx, sy, qx, qy, n_way):
    model.train()
    optimizer.zero_grad()
    sx, sy, qx, qy = (v.to(device) for v in (sx, sy, qx, qy))
    logits, per_query = logits_for_episode(model, sx, sy, qx, n_way)
...
def evaluate(model, split="test", n_episodes=200, n_way=5, k_shot=1):
    ...
    for _ in range(n_episodes):
        ...
        if sx is None:
            continue
        sx, sy, qx, qy = (v.to(device) for v in (sx, sy, qx, qy))
        logits, latency = logits_for_episode(model, sx, sy, qx, n_way)
        losses.append(F.cross_entropy(logits, qy).item())
        accs.append((logits.argmax(1) == qy).float().mean().item() * 100)
        times.append(latency * 1000)
    return dict(
        loss_mean=float(np.mean(losses)) if losses else 0.0,
        acc_mean=float(np.mean(accs)) if accs else 0.0,
        acc_std=float(np.std(accs)) if accs else 0.0,
        inf_ms_mean=float(np.mean(times)) if times else 0.0,
        n_episodes=len(accs),
    )
...
def main():
    ...
    for backbone in BACKBONES:
        for n_way, k_shot in TASKS:
            if result_exists(METHOD, backbone, n_way, k_shot):
                print("completed; skipping", backbone, n_way, k_shot)
                continue
            model = maybe_data_parallel(ProtoNet(backbone).to(device))
            optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
            start = 0
            latest = latest_path(METHOD, backbone, n_way, k_shot)
            if latest.exists():
                state = torch.load(latest, map_location=device, weights_only=False)
                load_portable_state_dict(model, state["model_state_dict"])
                optimizer.load_state_dict(state["optimizer_state_dict"])
                start = state["episode"] + 1
                print("resuming", latest, "at", start)
            for episode in range(start, episodes):
                sx, qx, sy, qy, *_ = sample_episode(
                    DATA_DIR, "train", n_way, k_shot, QUERY_PER_CLASS, TRANSFORM
                )
                if sx is None:
                    raise RuntimeError(
                        f"No valid {n_way}-way train episode found under {DATA_DIR}"
                    )
                loss, acc, latency = train_episode(
                    model, optimizer, sx, sy, qx, qy, n_way
                )
                append_episode(
                    METHOD, backbone, n_way, k_shot, episode, loss, acc, latency * 1000
                )
                if (episode + 1) % AUTOSAVE_EVERY == 0:
                    save_state(
                        _state(
                            model,
                            optimizer,
            ...
            metrics = evaluate(model, "test", EVAL_EPISODES, n_way, k_shot)
            state = _state(
                model,
                optimizer,
                episodes - 1,
            ...
            torch.save(portable_state_dict(model), CKPT_DIR / f"{METHOD}_{backbone}.pt")
