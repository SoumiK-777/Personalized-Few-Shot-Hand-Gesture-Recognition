...
from common.utils import set_seed,append_result
...
def run(kind,n,k,steps=0,lr=0):
 ...
 a=np.asarray(rows);append_result(method=kind+(f"_s{steps}_lr{lr}" if steps else ""),backbone=BACKBONE,n_way=n,k_shot=k,split="test",loss_mean=float(a[:,0].mean()),acc_mean=float(a[:,1].mean()),acc_std=float(a[:,1].std()),inf_ms_mean=float(a[:,2].mean()),n_episodes=len(rows))
def main():
 set_seed()
 for n,k in TASKS:
