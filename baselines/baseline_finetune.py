...
from common.config import DATA_DIR, EVAL_EPISODES, TASKS
from common.data import sample_episode
from common.utils import append_result, result_exists, set_seed
...
def run(kind, n, k, steps=0, lr=0):
    ...
    for _ in range(EVAL_EPISODES):
        ...
        if sx is None:
            continue
        sf, qf = embed(model, sx), embed(model, qx)
        sy, qy = sy.to(device), qy.to(device)
        start = time.perf_counter()
        if kind == "frozen_centroid":
            scores = -torch.cdist(
                qf, torch.stack([sf[sy == i].mean(0) for i in range(n)])
            )
        else:
            head = torch.nn.Linear(sf.shape[1], n).to(device)
            opt = torch.optim.Adam(head.parameters(), lr=lr)
            for _ in range(steps):
                opt.zero_grad()
                loss = F.cross_entropy(head(sf), sy)
                loss.backward()
                opt.step()
            scores = head(qf)
        latency = (time.perf_counter() - start) / len(qx) * 1000
        loss = F.cross_entropy(scores, qy).item()
        rows.append(
            (loss, (scores.argmax(1) == qy).float().mean().item() * 100, latency)
        )
    if not rows:
        raise RuntimeError(f"No valid test episodes for {method} | {n}-way {k}-shot")
    a = np.asarray(rows)
    append_result(
        method=method,
        backbone=BACKBONE,
...
def main():
    ...
    for n, k in TASKS:
        ...
        for s, lr in itertools.product((20, 50, 100), (1e-3, 1e-2)):
            run("linear_finetune", n, k, s, lr)
