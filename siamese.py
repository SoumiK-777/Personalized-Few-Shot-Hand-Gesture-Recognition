...
def pair_loss_and_scores(model, sx, sy, qx, qy, n, training=False):
    ...
    for qi, label in enumerate(qy.tolist()):
        for si, sl in enumerate(sy.tolist()):
            (positives if label == sl else negatives).append((qi, si, label == sl))
    # cap negatives to positives; this preserves a useful balanced signal instead of N-way cross-product growth
    if training:
        negatives = random.sample(negatives, min(len(negatives), len(positives)))
    pairs = positives + negatives
    qi = torch.tensor([p[0] for p in pairs], device=device)
    si = torch.tensor([p[1] for p in pairs], device=device)
    target = torch.tensor([p[2] for p in pairs], device=device, dtype=torch.float)
    dist = F.pairwise_distance(qf[qi], sf[si])
    loss = (target * dist.pow(2) + (1 - target) * F.relu(2.0 - dist).pow(2)).mean()
    start = time.perf_counter()
    distances = torch.cdist(qf, sf)
    class_dist = torch.stack([distances[:, sy == i].mean(1) for i in range(n)], 1)
    latency = (time.perf_counter() - start) / len(qx)
...
def train_episode(model, opt, sx, sy, qx, qy, n):
    ...
    loss, scores, t = pair_loss_and_scores(model, sx, sy, qx, qy, n, True)
    loss.backward()
    opt.step()
...
def evaluate(model, split="test", n_episodes=200, n_way=5, k_shot=1):
    ...
    for _ in range(n_episodes):
        ...
        if sx is None:
            continue
        sx, sy, qx, qy = (x.to(device) for x in (sx, sy, qx, qy))
        loss, s, t = pair_loss_and_scores(model, sx, sy, qx, qy, n_way)
        rows.append(
            (loss.item(), (s.argmax(1) == qy).float().mean().item() * 100, t * 1000)
        )
    ...
    for b in BACKBONES:
        for n, k in TASKS:
            if result_exists(METHOD, b, n, k):
                ...
                continue
            model = maybe_data_parallel(SiameseNetwork(b).to(device))
            opt = torch.optim.Adam(model.parameters(), lr=1e-3)
            latest = latest_path(METHOD, b, n, k)
            start = 0
            if latest.exists():
                s = torch.load(latest, map_location=device, weights_only=False)
                load_portable_state_dict(model, s["model_state_dict"])
            ...
            torch.save(portable_state_dict(model), CKPT_DIR / f"{METHOD}_{b}.pt")
