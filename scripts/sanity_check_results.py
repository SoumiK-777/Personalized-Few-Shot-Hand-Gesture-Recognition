...
def main():
    ...
    if output.exists():
        ...
        return
    p = RESULTS_DIR / "all_results.csv"
    rows = list(csv.DictReader(p.open())) if p.exists() else []
    seen = {}
    collisions = []
    for r in rows:
        key = (r["acc_mean"], r["acc_std"], r["inf_ms_mean"])
        cfg = (r["method"], r["backbone"], r["n_way"], r["k_shot"])
        if key in seen and seen[key] != cfg:
            collisions.append((seen[key], cfg, key))
        seen[key] = cfg
